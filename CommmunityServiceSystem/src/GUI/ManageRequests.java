...
public class ManageRequests extends JPanel {
    ...
    // START: MODIFICATIONS FOR BUTTON ACCESS AND CONTROL
    private RoundButton inProcessButton;
    private RoundButton doneButton;
    // END: MODIFICATIONS FOR BUTTON ACCESS AND CONTROL
    ...
    // Updated Colors for the new workflow (In Process and Done)
    ...
    public ManageRequests(JFrame parentFrame, CardLayout cardLayout, JPanel cardPanel) {
        ...
        contentPanel.add(createRequestDetailsPanel(), BorderLayout.CENTER); // Main details panel
        
        add(contentPanel, BorderLayout.CENTER);
        
        // Initializer for the panel displaying data from ApprovedRequests
        // Changed the initial call to match the new setRequestData signature
        setRequestData(0, "N/A", "N/A", "N/A", "Select an Approved request to manage.", null); 
    }

    /**
     * Public method to load request data when navigating from ApprovedRequests.
     * ADDED currentUpdateStatus parameter
     */
    public void setRequestData(int id, String category, String fullName, String unit, String description, String currentUpdateStatus) { // MODIFIED SIGNATURE
        this.requestId = id;
        ...
        // Update UI labels with the real request data
        if (id > 0) {
        ...
        // START: NEW LOGIC TO DISABLE BUTTONS (PRESS ONLY ONCE)
        if (inProcessButton != null && doneButton != null) {
            if (currentUpdateStatus == null || currentUpdateStatus.equals("Approved")) {
                // Initial state: Request is Approved (update_status is NULL), allow 'In Process'
                inProcessButton.setEnabled(true);
                doneButton.setEnabled(false); 
            } else if (currentUpdateStatus.equals("In Process")) {
                // Request is In Process, can only move to 'Done'
                inProcessButton.setEnabled(false);
                doneButton.setEnabled(true);
            } else if (currentUpdateStatus.equals("Done")) {
                // Request is Done, disable both buttons
                inProcessButton.setEnabled(false);
                doneButton.setEnabled(false);
            }
        }
        // END: NEW LOGIC TO DISABLE BUTTONS
    }
    
    // --- UI Components (Header and Nav remain unchanged) ---
    ...
    private JPanel createNavigationPanel() {
        ...
        // Navigates back to the main Requests dashboard, or ApprovedRequests list
    ...
    private JPanel createRequestDetailsPanel() {
        ...
        // --- Details Grid ---
        JPanel detailsGrid = new JPanel(new GridLayout(4, 2, 10, 10));
        ...
        // --- Action Buttons (In Process and Done) ---
        JPanel actionPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 30, 10));
        actionPanel.setBackground(WHITE_BG);
        
        // Button 1: In Process (Updates update_status)
        inProcessButton = new RoundButton("In Process", BLUE_IN_PROCESS); // Assigned to class field
        ...
        // Button 2: Done (Updates update_status)
        doneButton = new RoundButton("Done", GREEN_DONE); // Assigned to class field
    ...
    private void processRequestUpdate(String newUpdateStatus) {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            if (updateRequestUpdateStatus(newUpdateStatus)) {
                ...
                // Clear the current view and go back to the main requests dashboard
                setRequestData(0, "N/A", "N/A", "N/A", "Select an Approved request to manage.", null); // Updated call
    ...
    // --- Navigation Helper (remains unchanged) ---
