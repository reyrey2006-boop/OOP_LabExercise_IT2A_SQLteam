...
public class Requests extends JPanel {
    ...
    public Requests(JFrame parentFrame, CardLayout cardLayout, JPanel cardPanel) {
        ...
        topMetricPanel.add(createMetricCard("Pending Request", true));

        JPanel bottomMetricPanel = new JPanel(new GridLayout(1, 2, 20, 0));
        bottomMetricPanel.setOpaque(false);
        bottomMetricPanel.add(createMetricCard("Approved Requests", false));
        bottomMetricPanel.add(createMetricCard("Declined Requests", false));
    ...
    // 🔹 UPDATE LABEL TEXT (AUTO REFRESH)
    private void updateMetricLabels() {
        pendingLabel.setText(String.valueOf(pendingRequests));
        approvedLabel.setText(String.valueOf(approvedRequests));
        declinedLabel.setText(String.valueOf(declinedRequests));
    }

    // ================= ROUNDED CARD =================
    private JPanel createMetricCard(String title, boolean isPending) {
        ...
        if (title.equals("Pending Request")) {
            countLabel.setForeground(new Color(40, 167, 69));
            pendingLabel = countLabel;
            countLabel.setText(String.valueOf(pendingRequests));
        } else if (title.equals("Approved Requests")) {
            countLabel.setForeground(ACCENT_COLOR);
            approvedLabel = countLabel;
            countLabel.setText(String.valueOf(approvedRequests));
        } else {
            countLabel.setForeground(new Color(220, 53, 69));
            declinedLabel = countLabel;
            countLabel.setText(String.valueOf(declinedRequests));
        }
        ...
        card.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                switch (title) {
                    case "Pending Request" -> cardLayout.show(cardPanel, PENDING_REQUESTS_VIEW);
                    case "Approved Requests" -> cardLayout.show(cardPanel, APPROVED_REQUESTS_VIEW);
                    case "Declined Requests" -> cardLayout.show(cardPanel, DECLINED_REQUESTS_VIEW);
                }
            }
        });
